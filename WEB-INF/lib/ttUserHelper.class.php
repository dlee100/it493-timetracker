...
class ttUserHelper {
  ...
  static function insert($fields, $hash = true) {
    ...
    $sql = "insert into tt_users (name, login, password, group_id, org_id, role_id, client_id, rate, vacation_accrual_rate, quota_percent, email, created, created_ip, created_by $status_f) values (".
      $mdb2->quote($fields['name']).", ".$mdb2->quote($fields['login']).
      ", $password, $group_id, $org_id, ".$mdb2->quote($fields['role_id']).", ".$mdb2->quote($fields['client_id']).", $rate, $vacation_accrual_rate, $quota_percent, ".$mdb2->quote($email).", now() $created_ip_v $created_by_v $status_v)";
